...
\begin{frame}[plain]
    \vfill
    \begin{center}
        \uncover<1->{{\Huge\bfseries\color{accent} Agentic AI in FinTech\par} \vspace{0.4em} {\large\color{muted} Single-Agent vs Multi-Agent for Financial Reasoning\par}}
    \end{center}
...
\begin{frame}{Problem Overview}
    \begin{center}
        \uncover<1->{\textbf{Can LLMs answer live financial questions reliably?}}
    \end{center}
    \begin{itemize}
        \item<2-> Financial reasoning requires real-time data: prices, P/E ratios, sector lookups, news sentiment
        \item<3-> Training knowledge is stale --- hallucination risk is high without grounding
        \item<4-> 15 benchmark questions (easy / medium / hard) evaluated across Baseline, Single-Agent, and Multi-Agent
        \item<5-> Two independent implementations compared on gpt-4o-mini and gpt-4o
    \end{itemize}
    \uncover<6->{\begin{block}{Core Question} When does adding agent complexity actually improve over a bare LLM call? \end{block}}
\end{frame}
...
\begin{frame}{Key Components --- 7 Tools}
\begin{itemize}
...
\begin{frame}{Eugenio's Multi-Agent: Adaptive Router-Verifier}
    \uncover<1->{\flowpoint{Router classifies each question into domains (market / fundamentals / news) and selects only needed specialists.}}
    \uncover<2->{\flowpoint{Execution mode: parallel when independent, staged when one agent needs another's output first.}}
    \uncover<3->{\flowpoint{Evidence Verifier mechanically scores each specialist: tool usage, schema compliance, answer substance $\to$ confidence 0--1. No extra LLM call.}}
    \uncover<4->{\flowpoint{Aggregator merges results sorted by confidence --- higher-evidence sources win on conflicts.}}
\vspace{0.3em}
\centering
\scriptsize
\begin{tabular}{ll}
...
\begin{frame}{Oguz's Multi-Agent: Orchestrator-Critic-Synthesizer}
    \uncover<1->{\flowpoint{Orchestrator analyzes question, selects specialists, and detects ranking dependencies via structured JSON.}}
    \uncover<2->{\flowpoint{Two execution modes: single-phase (ThreadPoolExecutor) or two-phase when Phase 1 output feeds Phase 2.}}
    \uncover<3->{\flowpoint{LLM Critic (1 call per specialist) checks internal consistency --- do numbers in the answer match the raw tool output?}}
    \uncover<4->{\flowpoint{Synthesizer merges results: table format for multi-ticker comparisons, prose for others.}}
\vspace{0.3em}
\centering
\scriptsize
\begin{tabular}{ll}
...
\begin{frame}{Results --- gpt-4o-mini}
\begin{columns}[T]
...
\begin{frame}{Results --- Timing and Hallucinations}
\centering
\renewcommand{\arraystretch}{1.15}
...
\begin{frame}{Insights --- What Worked}
    \uncover<1->{\flowpoint{Tools are essential: baselines scored 0--8.9\% while tool-equipped agents reached 60--80\%.}}
    \uncover<2->{\flowpoint{Eugenio's mechanical Verifier kept latency low (5.8s single, 11.2s multi) while achieving the top score (80\%).}}
    \uncover<3->{\flowpoint{Oguz's LLM Critic produced zero hallucinations across all runs --- the extra LLM call for consistency checking paid off.}}
    \uncover<4->{\flowpoint{Both staged/two-phase execution modes correctly handled cross-domain chains (sector lookup $\to$ price $\to$ fundamentals).}}
\end{frame}

\begin{frame}{Insights --- What Failed and Lessons Learned}
    \uncover<1->{\flowpoint{Multi-Agent added 2$\times$ latency in every configuration. On gpt-4o-mini, Oguz's Multi (60\%) was slower than his Single (64.4\%) and less accurate.}}
    \uncover<2->{\flowpoint{Hard questions (multi-condition, cross-domain) remained the bottleneck: 40--67\% even in the best runs. Tool chaining errors accumulate.}}
    \uncover<3->{\flowpoint{Eugenio's mechanical verifier was cheaper but let through 1--3 hallucinations; Oguz's LLM Critic caught them all but cost extra API calls.}}
    \uncover<4->{\flowpoint{Complexity must earn its cost: multi-agent only outperformed single-agent in 2 of 4 runs (Eugenio 4o-mini, Oguz 4o).}}
\end{frame}

\begin{frame}[plain]
    \vfill
    \begin{center}
        \uncover<1->{{\large\color{accent} Demo --- Streamlit App\par} \vspace{0.5em} {\color{muted} Agent selector \textbullet\ Model selector \textbullet\ 3-turn follow-up \textbullet\ Context memory\par}}
    \end{center}
    \vfill
\end{frame}
